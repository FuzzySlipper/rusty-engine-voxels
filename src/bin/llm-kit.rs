...
use rusty_engine_voxels::flipbook::{FlipbookCompileSettings, compile_posed_flipbook, publish_compiled_flipbook};
use rusty_engine_voxels::kit::load_kit;
use voxel_asset::{encode_voxel_object, represented_voxel_count, with_computed_voxel_object_hashes};

fn main() -> Result<(), String> {
    let root = PathBuf::from("/home/dev/rusty-engine-voxels");
    let kit = load_kit(&root, "content/characters/directional-sentinel/character.json").map_err(|e| e.to_string())?;
    ...
    let voxels: Vec<AssembledVoxelCell> = vox_map.into_iter().enumerate().map(|(idx, (coord, slot))| AssembledVoxelCell{
        coordinate:coord,
        material_slot:slot,
        part_id:0,
    ...
    let mut compiled = compile_posed_flipbook(&kit, &[frame], &settings, source_bytes, 0.01).map_err(|e| e.to_string())?;
    compiled.asset.provenance.converter = "rusty-engine-voxels.llm-kit-v1".to_string();
    compiled.asset = with_computed_voxel_object_hashes(compiled.asset).map_err(|e| e.to_string())?;
    compiled.canonical_json = encode_voxel_object(&compiled.asset).map_err(|e| e.to_string())?;
    let publication = publish_compiled_flipbook(&root, "content/voxel-objects", &compiled).map_err(|e| e.to_string())?;
